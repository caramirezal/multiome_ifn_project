...
archerFolder <- '/media/sds-hd/sd21e005/binder_multiome/archer_folder_subset'


## Defining folder to store ouputs
figures_folder <- '/media/sds-hd/sd21e005/binder_multiome/multiome_ifn_project/figures/explorative_analysis_discarding_conditions'
...
sample.colors <- c(
        '7_3h_-IFN_polyC' = 'salmon',
        '5_3h_pIFN_polyC' = 'steelblue'
)

##--------------------------
## Subsetting to samples with good quality
## appeareance
subset <- c('7_3h_-IFN_polyC',
            '5_3h_pIFN_polyC')
...
pdf(paste0(figures_folder, '/umap_projection_all_samples_using_peaks_nfrags<5000_filtering_conditions.pdf'),
        height = 6,
        width = 6,
)
...
pdf(paste0(figures_folder, '/umap_projection_sequencing_bias_using_peaks_nfrags<5000_discarding_condtions.pdf'),
        height = 6,
        width = 6
)
p1
...
saveRDS(markerList, file = paste0(path2project, 'analysis/differential_peaks_polyC_3h.rds'))


markerList <- readRDS(file = paste0(path2project, 'analysis/differential_peaks_polyC_3h.rds'))


pdf(paste0(figures_folder, '/fdrVsLog2FC_peaks_5_3h_pIFN_polyC.pdf'), width = 6, height = 6)
samples <- names(sample.colors)
names(samples) <- samples
lapply(1:length(samples), 
       function(idx){
               markerList[idx][[1]] %>%
                       as.data.frame() %>%
                       ...
                       labs(title = samples[idx])
       })
dev.off()


pdf(paste0(figures_folder, '/peaksTracks_5_3h_pIFN_polyC.pdf'), width = 6, height = 6)
p <- plotBrowserTrack(
        ArchRProj = proj, 
        groupBy = "Sample", 
        geneSymbol = c("ISG15"),
